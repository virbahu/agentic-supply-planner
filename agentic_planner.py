...
class AgenticPlanner:
    def __init__(s, n=8, rate=25):
        s.n = n; s.rate = rate
    def simulate(s, periods=365, seed=42):
        rng = np.random.default_rng(seed)
        demand = np.maximum(rng.normal(400, 80, periods), 0)
        results = []; inv = 400*2; cost = 0
        for t in range(periods):
            d = demand[t]
            order = max(0, d*1.1 - inv + 400)
            inv = max(0, inv + order - d)
            holding = inv * 0.04
            shortage = max(0, d - inv) * 30
        ...
        return {{"total_cost": round(cost,0), "avg_inventory": round(np.mean([r["inventory"] for r in results]),1),
                ...
                "periods": len(results)}}
if __name__=="__main__": print(AgenticPlanner().simulate())
